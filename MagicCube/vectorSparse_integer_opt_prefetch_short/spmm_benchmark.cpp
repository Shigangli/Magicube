...
void BmFN(std::string benchmark, int dimK, int vec_length, int kernel, bool sorted, bool func, int sparse, int preA, int preB){
	...
	std::cout << " nonzero_vec: " << nonzeros_vec << " aligned_ nonzero_vec: " << aligned_num_item  << "\n" ;
        ...
        short *aligned_col_indices_shuffle = new short[aligned_num_item];
	...
	for(int i = 0; i < aligned_num_item/8; i++){
	    for(int j = 0; j < 8; j++){
	        aligned_col_indices_shuffle[i*8 + (j%2)*4 + j/2] = (short)(aligned_col_indices[i*8 + j]);
	    }
	}
	...
	flops = flops/1024.0/1024.0/1024.0;
        ...
        // Device
        int *d_row_offsets, *d_row_indices;
        short *d_col_indices;
	...
	TypeB *d_rhs_matrix;
        ...
        checkCuda(cudaMalloc(&d_col_indices, aligned_num_item * sizeof(short)));
	...
	if(mma_k_dim == 16){
            checkCuda(cudaMemcpy(d_col_indices, aligned_col_indices, aligned_num_item * sizeof(short), cudaMemcpyHostToDevice));
	}
	else if(mma_k_dim == 32){
            checkCuda(cudaMemcpy(d_col_indices, aligned_col_indices_shuffle, aligned_num_item * sizeof(short), cudaMemcpyHostToDevice));
	}
