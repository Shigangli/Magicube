...
def extract_duration_set(v, k, kernel, list_):
    ...
    for i in np.arange(args.start, args.end):
        benchmark = lines[i][:-6]
        file_kernel = get_file_name('/raid/datasets/dlmc/%s' % benchmark, k, v, kernel, True, 'spmm', 'None', 'half', False)
        file_dense = get_file_name('/raid/datasets/dlmc/%s' % benchmark, k, v, 'dense', True, 'spmm', 'None', 'half', False)
