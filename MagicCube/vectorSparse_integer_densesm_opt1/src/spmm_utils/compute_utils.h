...
namespace spmm{
    ...
    struct wmmaComputeUtils4_4bit_8v {
	    ...
	    lhs_fragment[0] = lhs_tile_[lane_id_+n_group_idx*32];
    ...
    // Tile_K=128 4bit v=8 sm opt
    template <typename VecType>
    struct wmmaComputeUtils4_4bit_8v_sm_opt {

        // Shared memory buffers
        const int* dense_tile_;
        const int* lhs_offset_;
	const unsigned char* compact_local_indices_offset_;
	unsigned char* compact_local_indices_sm_;
        // Register file fragment to accumulate results into
        int thread_id_x_;
        int* output_fragment_;

        // Constructor
        __device__ __forceinline__ wmmaComputeUtils4_4bit_8v_sm_opt(
            const int* dense_tile,
            const VecType* lhs_matrix,
	    const unsigned char* compact_local_indices,
	    unsigned char* compact_local_indices_sm,
	    int row_offset_vec,
	    int thread_id_x,
            int* output_fragment):
            dense_tile_(dense_tile),
            lhs_offset_(reinterpret_cast<const int *>(lhs_matrix) + row_offset_vec + thread_id_x),
	    compact_local_indices_offset_(compact_local_indices + row_offset_vec),
	    compact_local_indices_sm_(compact_local_indices_sm),
            thread_id_x_(thread_id_x),
            output_fragment_(output_fragment){}
        
        // Compute
        __device__ __forceinline__ void TileMAC(int sparse_offset){
            int lhs_fragment[1];
	    lhs_fragment[0] = __ldg(lhs_offset_ + sparse_offset);

            int rhs_fragment[16];
            int rhs_fragment_transpose[16];
            unsigned char indices[8];

	    int start_row_id = (thread_id_x_ % 4) * 8;
	    int lane_id = thread_id_x_ / 4;

            //#pragma unroll
	    //for(int i=0; i<8; i++){
	    //    rhs_fragment[i*2] = *(dense_tile_ + (start_row_id+i) * 16 + lane_id); 
	    //    rhs_fragment[i*2+1] = *(dense_tile_ + (start_row_id+i) * 16 + lane_id + 8); 
	    //}

            //#pragma unroll
	    //for(int i=0; i<8; i++){
            //    unsigned char row_id = __ldg(compact_local_indices_offset_ + sparse_offset + start_row_id + i);
	    //    rhs_fragment[i*2] = *(dense_tile_ + row_id * 16 + lane_id); 
	    //    rhs_fragment[i*2+1] = *(dense_tile_ + row_id * 16 + lane_id + 8); 
	    //}
            *(reinterpret_cast<int2 *>(indices)) = *(reinterpret_cast<const int2 *>(compact_local_indices_offset_ + sparse_offset + start_row_id));
            #pragma unroll
	    for(int i=0; i<8; i++){
	        rhs_fragment[i*2] = *(dense_tile_ + (*(indices+ i)) * 16 + lane_id); 
	        rhs_fragment[i*2+1] = *(dense_tile_ + (*(indices+ i)) * 16 + lane_id + 8); 
	    }

            //*(compact_local_indices_sm_ + thread_id_x_) = __ldg(compact_local_indices_offset_ + sparse_offset + thread_id_x_);
            //#pragma unroll
	    //for(int i=0; i<8; i++){
	    //    rhs_fragment[i*2] = *(dense_tile_ + (*(compact_local_indices_sm_ + start_row_id + i)) * 16 + lane_id); 
	    //    rhs_fragment[i*2+1] = *(dense_tile_ + (*(compact_local_indices_sm_ + start_row_id + i)) * 16 + lane_id + 8); 
	    //}
        ...
        __device__ __forceinline__ void TileMAC_Residual(int sparse_offset){
            int lhs_fragment[1];
	    lhs_fragment[0] = __ldg(lhs_offset_ + sparse_offset);
	    ...
	    //
            ...
            //#pragma unroll
	    //for(int i=0; i<8; i++){
            //    unsigned char row_id = *(compact_local_indices_sm_ + start_row_id + i);
	    //    if(row_id != 255){
	    //        rhs_fragment[i*2] = *(dense_tile_ + row_id * 16 + lane_id); 
	    //        rhs_fragment[i*2+1] = *(dense_tile_ + row_id * 16 + lane_id + 8);
	    //    }	
