    ...
    for (int i = 0; i < m_vec; i++){
        ...
        for (int j = row_offsets[i*2]; j < row_offsets[i*2+1]; j++){
			...
			accumulator += (a_val*b_val); 
                ...
                ref_C[j*vec_length + v] = accumulator;
            }
        }
    }
    return flops;
...
void BmFN(std::string benchmark, int dimK, int vec_length, bool sorted, bool func, int sparse, int preA, int preB){
	...
	int *rhs_matrix;
        ...
        half *output_values = new half[aligned_num_item*vec_length];
	for(int i = 0; i < aligned_num_item*vec_length; i++){
        ...
        if (func){
            ...
            flops = Host_sddmm_integers(lhs_matrix, rhs_matrix, h_output_values, m, k, n, preA, preB, vec_length, aligned_row_offsets, aligned_col_indices, m_vec, 8);
	}
        ...
        checkCuda(cudaMalloc(&d_output_values, aligned_num_item*vec_length*sizeof(half)));
        ...
        checkCuda(cudaMemcpy(d_output_values, output_values, aligned_num_item*vec_length*sizeof(half), cudaMemcpyHostToDevice));
        ...
        if (preA == 4 && preB == 4){
            ...
            for(int iter=0; iter<NUM_PROFILES; ++iter){
                ...
                sddmm::wmmaSddmm_4b(m_vec, k, n, 1024.0, d_row_indices, d_row_offsets, d_col_indices, d_lhs_matrix, d_rhs_matrix, d_output_values, vec_length);
            ...
            for(int iter=0; iter<NUM_PROFILES; ++iter){
                ...
                cudaEventCreate(&sddmm_end);
                cudaEventRecord(sddmm_start);
                sddmm::wmmaSddmm_8b(m_vec, k, n, 1024.0, d_row_indices, d_row_offsets, d_col_indices, d_lhs_matrix, d_rhs_matrix, d_output_values, vec_length);
                cudaEventRecord(sddmm_end);
                cudaEventSynchronize(sddmm_end);
                cudaEventElapsedTime(&sddmm_ms, sddmm_start, sddmm_end);
                cudaEventDestroy(sddmm_start);
            ...
            for(int iter=0; iter<NUM_PROFILES; ++iter){
                ...
                cudaEventCreate(&sddmm_end);
                cudaEventRecord(sddmm_start);
                sddmm::wmmaSddmm_16b(m_vec, k, n, 1024.0, d_row_indices, d_row_offsets, d_col_indices, d_lhs_matrix, d_rhs_matrix, d_output_values, vec_length);
                cudaEventRecord(sddmm_end);
                cudaEventSynchronize(sddmm_end);
                cudaEventElapsedTime(&sddmm_ms, sddmm_start, sddmm_end);
                cudaEventDestroy(sddmm_start);
	...
	flops = flops/1024.0/1024.0/1024.0;
        ...
        if (func){
            ...
            half *output_value_cuda = new half[aligned_num_item*vec_length];
            checkCuda(cudaMemcpy(output_value_cuda, d_output_values, aligned_num_item*vec_length*sizeof(half), cudaMemcpyDeviceToHost)); 
		...
		//if(j<256)
                ...
                if ((output_value_cuda[j] - __float2half((float)(h_output_values[j])/1024.0)) != 0.0){
		    if(j<256)
                        printf("item %d, expect %f, got %f\n", j, __float2half((float)(h_output_values[j])/1024.0), output_value_cuda[j]);
                    errors ++;
                }
            }
            if (errors > 0) {
...
int main(int argc, char **argv){
	...
	//else if ((preA == 16) && (preB == 16)) BmFN(benchmark, dimK, vec_length, sorted, func, sparse, preA, preB); //TODO: support transpose
