...
namespace spmm{
    ...
    //larger Tile_K = 128 4bit v=8 sm opt
    template<typename OutType>
    struct wmmaOutputTile4_4bit_8v_sm_opt{
        //
        // Member variables
        //
        int lane_id_;
        ...
        // Constructor
        __device__ __forceinline__ wmmaOutputTile4_4bit_8v_sm_opt(
            int thread_id_x, int vec_length,
            int row_offset_vec, int column_offset,
            int col_num,
            int* output_fragment,
            OutType* output_matrix)
	    ...
	    //vec_length = 8 ???
            const int output_offset = (row_offset_vec * vec_length + thread_id_x / 4) * col_num + column_offset;
            output_matrix_ = reinterpret_cast<int4 *>(output_matrix + output_offset);
            lane_id_ = (thread_id_x % 4) * 4;
            //rhs_columns_int4 = col_num / 4;
	    ...
	    //}
            ...
            *(output_matrix_ + lane_id_ + 0) = *(output_fragment_ + 0);
            *(output_matrix_ + lane_id_ + 1) = *(output_fragment_ + 1);
            *(output_matrix_ + lane_id_ + 2) = *(output_fragment_ + 2);
            *(output_matrix_ + lane_id_ + 3) = *(output_fragment_ + 3);

            *(output_matrix_ + lane_id_ + 16) = *(output_fragment_ + 4);
            *(output_matrix_ + lane_id_ + 17) = *(output_fragment_ + 5);
            *(output_matrix_ + lane_id_ + 18) = *(output_fragment_ + 6);
            *(output_matrix_ + lane_id_ + 19) = *(output_fragment_ + 7);
