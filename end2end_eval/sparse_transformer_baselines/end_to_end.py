...
def sparse_profile():
    ...
    if args.mem:
        ...
        for i in range(32):
            out = spTrans(x)
        end_time = time.time()
        print("vectorSparse average runtime %.4f milliseconds, batchsize [%d]" % ((end_time-start_time)/32.0*1000.0, args.bs))
...
def dense_prof():
    ...
    if args.mem:
        ...
        for i in range(32):
            out = denseTrans(x)
        end_time = time.time()
        print("Dense average runtime %.4f milliseconds, batchsize [%d]" % ((end_time-start_time)/32.0*1000.0, args.bs))
