...
void BmFN(std::string benchmark, int dimK, int vec_length, int kernel, int alg, bool sorted, bool func, int sparse){
	...
	int NUM_PROFILES = 512;
        ...
        cudaProfilerStop();
	std::cout << "vectorSparse SDDMM runtime " << sddmm_ms_avg/(float)NUM_PROFILES << " ms" << "\n";
    ...
    // CuBLAS Dense GeMM
        ...
        char *lhs_matrix = new char[m * k];
        char *rhs_matrix = new char[n * k];
        MakeDenseMatrix<char>(m, k, lhs_matrix, generator);
	...
	int beta = 0;
        ...
        checkCuda(cudaMalloc(&d_lhs_matrix, (m * k) * sizeof(char)));
        checkCuda(cudaMalloc(&d_rhs_matrix, (n * k) * sizeof(char)));
        checkCuda(cudaMalloc(&d_output_values, (m * n) * sizeof(int)));
        
        checkCuda(cudaMemcpy(d_lhs_matrix, lhs_matrix, (m * k) * sizeof(char), cudaMemcpyHostToDevice));
        checkCuda(cudaMemcpy(d_rhs_matrix, rhs_matrix, (n * k) * sizeof(char), cudaMemcpyHostToDevice));

        cudaProfilerStart();
        printf("int8 Dense Baseline\n");
	float dense_ms_avg = 0.0f;
	int NUM_PROFILES = 512;
	for(int iter=0; iter<16; ++iter){
	    cublasGemmEx(handle, CUBLAS_OP_T, CUBLAS_OP_N, n, m, k, &alpha, d_rhs_matrix, CUDA_R_8I, k, d_lhs_matrix, CUDA_R_8I, k, &beta, d_output_values, CUDA_R_32I, n, CUDA_R_32I, CUBLAS_GEMM_DEFAULT_TENSOR_OP);
	}
	for(int iter=0; iter<NUM_PROFILES; ++iter){
	    float dense_ms = 0.0f;
        ...
        cudaProfilerStop();
	std::cout << "Int8 sddmm dense runtime " << dense_ms_avg/(float)NUM_PROFILES << " ms" << "\n";
    }
    else{
        // Create cublas handles
        cublasHandle_t handle;
	...
	for(int iter=0; iter<NUM_PROFILES; ++iter){
        ...
        cudaProfilerStop();
	std::cout << "F16 sddmm dense runtime " << dense_ms_avg/(float)NUM_PROFILES << " ms" << "\n";
...
int main(int argc, char **argv){
    ...
    if (argc == 2 && (strcmp(argv[1], "-h") == 0 || strcmp(argv[1], "--help") == 0)){
        ...
        if(sparse == 1){
	    if(alg == 0) printf("wmma + sm\n");
	    if(alg == 1) printf("mma + register\n");
	    if(alg == 2) printf("mma + warp shfl\n");
	}
	if(sparse == 0)
	    printf("cublase f16 dense kernel \n");
	if(sparse == -1)
	    printf("cublase int8 dense kernel \n");
