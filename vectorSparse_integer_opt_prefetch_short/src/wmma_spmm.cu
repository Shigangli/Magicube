    ...
    OutType* __restrict__ output_matrix)
{
    ...
    __shared__ short column_indices_tile_array[Tile_N*2];
    ...
    // Pointers to the shared memory tiles
    int* values_tile = values_tile_array;
    short* column_indices_tile = column_indices_tile_array;
    int* dense_tile = dense_tile_array;
    ...
    wmmaDenseTile_4bit<LoadType, Tile_N, Tile_K, BlockWidth> dense_tile_loader(
        k/8, k_index/8, lane_id, rhs_matrix, column_indices_tile, dense_tile, rhs_prefetch 
    );
    ...
    //BlockWidth should be equal to Tile_N
    int steps = nonzeros / Tile_N;
    int residue = nonzeros % Tile_N;

    if(steps > 0){
        sparse_tile_loader.Load(0, steps);
        __syncthreads();
        ...
        for(; i < steps; i++){
            ...
            sparse_tile_loader.Load(i, steps);
            __syncthreads();
    ...
    int* __restrict__ output_matrix)
{
    ...
    return wmmaSpmmEx_4bit<short, int, 1, 32, 128, 32>(m_vec, vec_length, k, n, row_indices, row_offsets, column_indices, values, rhs_matrix, output_matrix);
}
    ...
    int* __restrict__ output_matrix)
{
    //printf("4-bit wmmaSpmm\n");
    //return wmmaSpmmEx_4bit<int, 1, 64, 64, 32>(m_vec, vec_length, k, n, row_indices, row_offsets, column_indices, values, rhs_matrix, output_matrix);
    return wmmaSpmmEx_4bit<short, short, 1, 64, 128, 32>(m_vec, vec_length, k, n, row_indices, row_offsets, column_indices, values, rhs_matrix, output_matrix);
}
