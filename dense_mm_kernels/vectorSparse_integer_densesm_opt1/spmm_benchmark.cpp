...
void BmFN(std::string benchmark, int dimK, int vec_length, int kernel, bool sorted, bool func, int sparse, int preA, int preB, int rnum){
	...
	bool check_correct;
        ...
        for(int i = 0; i < row_chunk_offset[row_chunks]+1; i++){
	    //printf("compact block offset: %d ", compact_block_offset[i]);
	    if(i+1 <= row_chunk_offset[row_chunks])
	        //printf("compact block size: %d\n", compact_block_offset[i+1]-compact_block_offset[i]);
		if((compact_block_offset[i+1]-compact_block_offset[i]) < 10 || (compact_block_offset[i+1]-compact_block_offset[i]) > 128)
	            printf("compact block size: %d\n", compact_block_offset[i+1]-compact_block_offset[i]);
	}

        for(int i = 0; i < row_chunks+1; i++){
	    printf("row chunk offset: %d ", row_chunk_offset[i]);
	    if(i+1 <= row_chunks)
	        printf("row chunk size: %d \n", row_chunk_offset[i+1]-row_chunk_offset[i]);
	}

	printf("row num %d, compact ind num %d, org %d, save %f \n", row_num, compact_index_counter, nonzeros_vec, (float)(nonzeros_vec - compact_index_counter)/nonzeros_vec);
	...
	flops = flops/1024.0/1024.0/1024.0;
        ...
        // Device
        int *d_row_offsets, *d_col_indices, *d_row_indices, *d_row_chunk_offset, *d_compact_block_offset;
        unsigned char *d_compact_local_indices;
	...
	TypeB *d_rhs_matrix;
        ...
        checkCuda(cudaMalloc(&d_col_indices, compact_index_counter * sizeof(int)));
        checkCuda(cudaMalloc(&d_row_indices, m_vec * sizeof(int)));
        checkCuda(cudaMalloc(&d_row_chunk_offset, (row_chunks+1) * sizeof(int)));
        checkCuda(cudaMalloc(&d_compact_block_offset, (row_chunk_offset[row_chunks]+1) * sizeof(int)));
        checkCuda(cudaMalloc(&d_compact_local_indices, aligned_num_item * sizeof(unsigned char)));
	...
	if(mma_k_dim == 16){
            ...
            checkCuda(cudaMemcpy(d_col_indices, compact_col_indices, compact_index_counter * sizeof(int), cudaMemcpyHostToDevice));
	}
        ...
        checkCuda(cudaMemcpy(d_row_chunk_offset, row_chunk_offset, (row_chunks+1) * sizeof(int), cudaMemcpyHostToDevice));
        checkCuda(cudaMemcpy(d_compact_block_offset, compact_block_offset, (row_chunk_offset[row_chunks]+1) * sizeof(int), cudaMemcpyHostToDevice));
        checkCuda(cudaMemcpy(d_compact_local_indices, compact_local_indices_shuffle, aligned_num_item * sizeof(unsigned char), cudaMemcpyHostToDevice));
	...
	int NUM_PROFILES = 512;
	    ...
	    for(int iter=0; iter<NUM_PROFILES; ++iter){
	        ...
	        cudaEventRecord(spmm_start);
                ...
                spmm::wmmaSpmm_4b8v_sm_opt(m_vec, vec_length, k, n, row_num, d_row_indices, d_row_offsets, d_col_indices, d_row_chunk_offset, d_compact_block_offset, d_compact_local_indices, d_value, d_rhs_matrix, d_output_value);
...
int main(int argc, char **argv){
    ...
    // Run the benchmark
        ...
        int rnum = std::atoi(argv[10]);

	if ((preA == 8) && (preB == 8) && (vec_length == 4)) BmFN<int, int, int, short, half2, short2, CUDA_R_16F>(benchmark, dimK, vec_length, kernel, sorted, func, sparse, preA, preB, rnum);
	else if ((preA == 4) && (preB == 4) && (vec_length == 4)) BmFN<short, int, int, short, half2, short2, CUDA_R_16F>(benchmark, dimK, vec_length, kernel, sorted, func, sparse, preA, preB, rnum);
	else if ((preA == 4) && (preB == 4) && (vec_length == 8)) BmFN<int, int, int, short, half2, short2, CUDA_R_16F>(benchmark, dimK, vec_length, kernel, sorted, func, sparse, preA, preB, rnum);
