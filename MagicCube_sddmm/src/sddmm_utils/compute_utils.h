namespace sddmm{
    ...
    struct wmmaComputeUtils_4b{
        ...
        __device__ __forceinline__ void TileMAC(int step){
            //int lhs_fragment[Blocks] = {0, 0};
            int lhs_fragment[Blocks] = {};
            //int rhs_fragment[2] = {};
	    //if(warp_id_ == 0){
	    //    rhs_fragment[0] = rhs_prefetch_[(step % 2) * 2 + 0];
	    //    rhs_fragment[1] = rhs_prefetch_[(step % 2) * 2 + 1];
	    //}else{
	    //    rhs_fragment[0] = lane_id_;
	    //    rhs_fragment[1] = lane_id_ % warp_id_;
	    //}

            int *rhs_fragment = rhs_prefetch_ + (step % 2) * 2;
	    ...
	    //if(lane_id_ % 32 < ValuesBlockWidth){
            //    #pragma unroll
	    //    for(int i=0; i<Blocks; i++){
	    //        lhs_fragment[i] = lhs_tile_[i*ValuesBlockWidth + lane_id_ % 32];
	    //    }
	    //}
