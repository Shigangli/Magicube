namespace sddmm {
    template <int BlockWidth, int Tile_K_int, int VecLength, int MMA_Dim_K_int>
    struct wmma_lhs_4b{
        const int dim_k_;
        const int lane_id_;
	    ...
	    if(lane_id_ < BlockWidth){
		lhs_prefetch_[0] = __ldg(lhs_matrix_base_ + offset_int + (lane_id_ / (MMA_Dim_K_int * VecLength)) * MMA_Dim_K_int + lane_id_ % (MMA_Dim_K_int * VecLength) / MMA_Dim_K_int * dim_k_ + lane_id_ % MMA_Dim_K_int);
	    }
        }
