...
namespace spmm {
    ...
    struct wmmaDenseTile_8b{
		...
		//bank-conflict free
                *(dense_tile_ + i*72 + lane_id_ % 64 + (lane_id_ / 64) * 288) = __ldg(matrix_base_ + *(row_offsets + i*4)*rhs_cols_ + global_offset);
                //*(dense_tile_ + i*64 + lane_id_ % 64 + (lane_id_ / 64) * 256) = __ldg(matrix_base_ + *(row_offsets + i*4)*rhs_cols_ + global_offset);
            }
        }
    ...
    struct wmmaDenseTile_4b{
	    ...
	    //conflict-free
            ...
            //}
            for(int i=0; i<8; i++){
                *(dense_tile_ + lane_id_ + i*64) = rhs_prefetch_[i];
            }
        }
        ...
        __device__ __forceinline__ void Fetch(int step){
            ...
            const int bank_id = lane_id_%16;
            for(int i=0; i<8; i++){
                *(dense_tile_ + lane_id_ + i*64) = __ldg(matrix_base_ + *(row_offsets + i*4)*rhs_cols_ + bank_id);
            }
        }
