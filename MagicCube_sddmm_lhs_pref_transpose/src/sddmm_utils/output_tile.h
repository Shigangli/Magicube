namespace sddmm{
    ...
    struct wmmaOutputTile_4b{
        ...
        __device__ __forceinline__ void Store(){
		    ...
		    *(output_values_base_ + (lane_id_ % 4) * VecLength * 2 + (lane_id_ % 32) / 4 + warp_id_ * output_warp_width_) = output_fragment_[0];
		    *(output_values_base_ + (lane_id_ % 4) * VecLength * 2 + VecLength + (lane_id_ % 32) / 4 + warp_id_ * output_warp_width_) = output_fragment_[1];
		}
	    }
        }
    };
