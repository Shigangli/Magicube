namespace sddmm{
    template <int ValuesBlockSize, int Blocks>
    struct wmmaComputeUtils_4b{
        ...
        __device__ __forceinline__ void TileMAC(int step){
	        ...
	        if(lane_id_ % 32 < ValuesBlockSize){
                    #pragma unroll
	            for(int i=0; i<Blocks; i++){
	                lhs_fragment[i] = lhs_tile_[i*ValuesBlockSize + lane_id_ % 32 + (step % 2) * ValuesBlockSize * Blocks];
	            }
	        }
    ...
    template <int ValuesBlockSize, int Blocks, int PAD_chunk_size, int Tile_K_int>
    struct wmmaComputeUtils_8b{
        ...
        __device__ __forceinline__ void TileMAC(int step){
	        ...
	        if(lane_id_ % 32 < ValuesBlockSize){
                    #pragma unroll
	            for(int i=0; i<Blocks; i++){
	                lhs_fragment[i] = lhs_tile_[i*4 + lane_id_%4 + lane_id_%32/4 * Tile_K_int + lane_id_ %32/4/2*4 + (step % 2) * (ValuesBlockSize * Blocks + PAD_chunk_size)];
	            }
	        }
    ...
    struct wmmaComputeUtils_16b{
        ...
        __device__ __forceinline__ void TileMAC(int step){
	        ...
	        if(lane_id_ % 32 < ValuesBlockSize){
                    #pragma unroll
	            for(int i=0; i<Blocks; i++){
	                lhs_fragment[i*2 + 0] = lhs_tile_[i*ValuesBlockSize + lane_id_ % (ValuesBlockSize/2)*2 + (step % 2) * ValuesBlockSize * Blocks];
	                lhs_fragment[i*2 + 1] = lhs_tile_[i*ValuesBlockSize + lane_id_ % (ValuesBlockSize/2)*2 + 1 + (step % 2) * ValuesBlockSize * Blocks];
	            }
	        }
		...
		unsigned char *rhs_fragment_decompose_char = reinterpret_cast<unsigned char *>(rhs_fragment_decompose);
                ...
                if(lane_id_ % 32 < ValuesBlockSize){
		    if(lane_id_ % 32 < ValuesBlockSize/2){
                        #pragma unroll
    ...
    struct wmmaComputeUtils_16b8v{
        ...
        __device__ __forceinline__ void TileMAC(int step){
	        ...
	        if(lane_id_ % 32 < ValuesBlockSize){
                    #pragma unroll
	            for(int i=0; i<Blocks; i++){
	                //lhs_fragment[i*2 + 0] = lhs_tile_[i*(ValuesBlockSize+2) + lane_id_ % (ValuesBlockSize/2)*2 + lane_id_ % (ValuesBlockSize/2)/16 + (step % 2) * (ValuesBlockSize * Blocks + 4)];
	                //lhs_fragment[i*2 + 1] = lhs_tile_[i*(ValuesBlockSize+2) + lane_id_ % (ValuesBlockSize/2)*2 + lane_id_ % (ValuesBlockSize/2)/16 + 1 + (step % 2) * (ValuesBlockSize * Blocks + 4)];
	                lhs_fragment[i*2 + 0] = lhs_tile_[i*ValuesBlockSize + lane_id_ % (ValuesBlockSize/2)*2 + (step % 2) * ValuesBlockSize * Blocks];
	                lhs_fragment[i*2 + 1] = lhs_tile_[i*ValuesBlockSize + lane_id_ % (ValuesBlockSize/2)*2 + 1 + (step % 2) * ValuesBlockSize * Blocks];
	            }
	        }
