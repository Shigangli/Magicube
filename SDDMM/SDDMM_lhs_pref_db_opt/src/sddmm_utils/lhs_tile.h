namespace sddmm {
    ...
    template <int BlockWidth, int Tile_K_int, int WarpBlockWidth, int MMA_Dim_K_int, int PAD_chunk_size>
    struct wmma_lhs_8b{
        const int dim_k_;
        const int lane_id_;
        const int *lhs_matrix_base_;
        ...
        __device__ __forceinline__ void LoadRowfromRegister(int step){
	    if(lane_id_ < BlockWidth){
		lhs_tile_[lane_id_ + lane_id_/32 *4 + (step % 2) * (BlockWidth + PAD_chunk_size)] = lhs_prefetch_[0];
	    }
        }
	    ...
	    if(lane_id_ < BlockWidth){
		//lhs_prefetch_[0] = __ldg(lhs_matrix_base_ + offset_int + lane_id_ / WarpBlockWidth * MMA_Dim_K_int + lane_id_ % WarpBlockWidth / MMA_Dim_K_int * dim_k_ + lane_id_ % MMA_Dim_K_int);
		lhs_prefetch_[0] = __ldg(lhs_matrix_base_ + offset_int + lane_id_ / Tile_K_int * dim_k_ + lane_id_ % Tile_K_int);
	    }
        }
