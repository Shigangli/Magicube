    ...
    int* __restrict__ output_values)
{
    ...
    __shared__ int lhs_tile_array[Tile_K*VecLength/2 + (VecLength-1)*4];
    __shared__ int column_indices_tile_array[Tile_N];
    ...
    wmma_lhs_8b<Tile_K * VecLength / 4, Tile_K / 4, VecLength * 4, 4, VecLength * 2> lhs_loader(
    ...
    // Accumulator registers for the output values.
    __align__(16) int output_fragment[2] = {};
    wmmaComputeUtils_8b<4 * VecLength, Tile_K / 16, VecLength * 2, Tile_K / 4> computer(lane_id, workset, lhs_tile, rhs_prefetch, output_fragment);
